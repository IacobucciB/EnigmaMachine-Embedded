/* Version V1.0.9
 ...
 It is responsibility of your program to deal with converting special cases
...
void GPIO0_IRQHandler(void) {
	...
	{
		//Probar ponerlo al final si se producen errores!!!
...
void send_bit(void) {
	...
	case 9:
		...
		gpioWrite(PS2_DataPin, val); // send start bit
		_parity += val;            // another one received ?
		_shiftdata >>= 1;          // right _SHIFT one place for next bit
		break;
	case 10:
		// Parity - Send LSB if 1 = odd number of 1's so parity should be 0
		gpioWrite(PS2_DataPin, (~_parity & 1));
		break;
...
uint16_t translate(void) {
	...
	if (retdata > 0) {
		...
		} else if (retdata >= PS2_KEY_L_SHIFT && retdata <= PS2_KEY_R_GUI) { // Update bits for _SHIFT, _CTRL, _ALT, _ALT GR, _GUI in status
#if defined( PS2_REQUIRES_PROGMEM )
...
/*  Get the ID used in keyboard
 returned data in keyboard buffer read as keys */
void PS2KeyAdvanced_readID(void) {
...
/*  Get the current Scancode Set used in keyboard
 returned data in keyboard buffer read as keys */
void PS2KeyAdvanced_getScanCodeSet(void) {
...
uint8_t PS2KeyAdvanced_getLock() {
	return (PS2_led_lock);
}

/* Sets the current status of Locks and LEDs */
void PS2KeyAdvanced_setLock(uint8_t code) {
...
/* Set library to not send break key codes
 1 = no break codes
 0 = send break codes  */
void PS2KeyAdvanced_setNoBreak(uint8_t data) {
	_mode &= ~_NO_BREAKS;
...
/* Set library to not repeat make codes for _CTRL, _ALT, _GUI, _SHIFT
 1 = no repeat codes
 0 = send repeat codes  */
void PS2KeyAdvanced_setNoRepeat(uint8_t data) {
	_mode &= ~_NO_REPEATS;
...
/*  Send Typematic rate/delay command to keyboard
 ...
 -5 parameter error
 */
int PS2KeyAdvanced_typematic(uint8_t rate, uint8_t delay) {
	if (rate > 31 || delay > 3)
		return -5;
...
uint8_t PS2KeyAdvanced_available() {
	int8_t i, idx;
	uint16_t data;
...
uint16_t PS2KeyAdvanced_read( )
{
	uint16_t result;
	uint8_t idx;

	result = PS2KeyAdvanced_available();
	if( result )
	{
		idx = _key_tail;
		idx++;
...
void PS2KeyAdvanced_begin(gpioMap_t dataPin, gpioMap_t irqPin, uint8_t intChannel) {
	/* PS2 variables reset */
	ps2_reset();
	...
	// initialize the pins
	gpioInit( PS2_DataPin, GPIO_INPUT_PULLUP ); /* Setup Data pin */
	...
	Chip_PININT_SetPinModeEdge( LPC_GPIO_PIN_INT, PININTCH(PS2_IntChannel) );	// Selecciona activo por flanco (edge-sensitive)
	Chip_PININT_EnableIntLow( LPC_GPIO_PIN_INT, PININTCH(PS2_IntChannel) );		// Selecciona activo por flanco descendente

	NVIC_SetPriority( PIN_INT0_IRQn + PS2_IntChannel, 7 );
	NVIC_ClearPendingIRQ( PIN_INT0_IRQn + PS2_IntChannel );
	NVIC_EnableIRQ( PIN_INT0_IRQn + PS2_IntChannel );
}
